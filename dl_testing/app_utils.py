...
def do_inference(unique_id, model_name, url):
...
def download_model(unique_id, model_name):
    ...
    blob_path = "/AppData/" + str(unique_id) + "/models/" + str(model_name) + "/model/frozen_inference_graph.pb"
    ...
    with open('models/'+ model_name + '/frozen_inference_graph.pb', 'wb') as data:
        try:
    ...
    # Writing the data in the file to local folder
    with open('models/'+ model_name + '/label_map.pbtxt', 'wb') as data:
        try:
            # Downloading stream of config data
            download_stream = blob_client.download_blob()
            # Writing it in a config file
...
def upload_infered_images(img, unique_id, model_name, img_url):
    ...
    blob_client = blob_service_client.get_blob_client(container=oiazurecred['blobcontainername'], 
                                    blob= "/AppData/" +str(unique_id) + "/inference/models/" + str(model_name) + "/Images/" + img_name)
